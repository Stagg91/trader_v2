...
class PaperTrader(BybitClient):
    ...
    def open_trade(self, symbol: str, side: str, qty: float, order_type: str = "Market", price: float = None, category: str = "linear", stop_loss: float = None, take_profit: float = None, trailing_stop: float = None):
        ...
        """
        sl_msg = f"SL: {stop_loss}" if stop_loss else ""
        tp_msg = f"TP: {take_profit}" if take_profit else ""
        ts_msg = f"TS: {trailing_stop}" if trailing_stop else ""
        print(f"[PAPER] Open Trade: {side} {qty} {symbol} @ {price or 'Market'} {sl_msg} {tp_msg} {ts_msg}")
        # Log to TradeLog?
