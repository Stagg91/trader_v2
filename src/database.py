...
class Settings(Base):
    ...
    last_pnl_reset = Column(Float, default=0.0)
...
def init_db():
    ...
    if inspector.has_table("settings"):
        ...
        if "daily_pnl" not in columns:
             print("Migrating settings table: adding risk persistence...")
             with engine.connect() as conn:
                try:
                    conn.execute(text("ALTER TABLE settings ADD COLUMN daily_pnl FLOAT DEFAULT 0.0"))
                    conn.execute(text("ALTER TABLE settings ADD COLUMN last_pnl_reset FLOAT DEFAULT 0.0"))
                    conn.commit()
                except Exception as e:
                     print(f"Migration Error: {e}")
